...
sub new
  ...
  RegisterFunction('DbSaveStateOfCharge',\&DbSaveStateOfCharge);
...
sub DbSaveStateOfCharge
  {
  my ($timestamp, $recordtype, $recordnumber, $ownername, $vehicleid, $data, $expires) = @_;

  $db->do('INSERT INTO ovms_dopey (owner,vehicleid,h_timestamp,h_recordtype,h_recordnumber,h_data,h_expires) '
        . 'VALUES (?,?,?,?,?,?,?) '
